...
| Evidence | Result |
|---|---|
| `git log --oneline -6` | Branch `codex/fieldtheory-suite` is pushed through `98ff91d feat(hosted): add readiness operator actions`. |
| `gh pr checks 1 --repo chipoto69/fieldtheory` | `preview` passes and CodeRabbit reports `pass` / review skipped. |
| `npm run hosted:check-readiness -- --remote --json` | Blocked only on missing required production secret names: `VERCEL_TOKEN`, `DATABASE_URL`, `PRIVY_APP_ID`, `NEXT_PUBLIC_PRIVY_APP_ID`, `PRIVY_APP_SECRET`; optional `PRIVY_JWT_VERIFICATION_KEY` is missing. |
...
|---|---|---|
| Deploy/auth readiness | Production deploy is blocked by five missing operator-owned secrets; Privy linked identity policy is scaffolded but not a hard production gate. | Keep production deploy blocked until secrets, dashboard policy, and authenticated mutation smoke are recorded. |
...
3. Production linked-identity policy is not yet enforced by readiness. If this
   is mandatory for launch, `FIELD_THEORY_REQUIRE_LINKED_IDENTITIES=true`,
   Base chain id, and Solana cluster must become release gates.
4. Target Postgres has no production migration proof, schema-version output,
   backup/restore drill, or audit retention policy.
5. Production smoke proves public routes and unauthenticated `401`, but not an
